     ...
     &     disl1,disl2,rlon,rlat,utm,dstmx,edsp,ndsp,zdsp,m,n)
c***********************************************************************
c   
c***********************************************************************
...
c Loop over faults
      do 200 i=1,n
c Translate the origin from fault center to lower left corner
         tmp2 = cos(dip(i)*dtr)
         tmp1 = 0.5*sqrt(length(i)*length(i) + tmp2*wdt(i)*tmp2*wdt(i))
         tmp2 = atan2(tmp2*wdt(i),length(i))
         odep = edep(i) + 0.5*wdt(i)*sin(dip(i)*dtr)
         if (utm) then
            oy = elat(i) - 1000.*tmp1*cos(tmp2-strk(i)*dtr)
            ox = elon(i) + 1000.*tmp1*sin(tmp2-strk(i)*dtr)
c            write(*,*) 'Origin (UTM) of subfault ',i,': ',ox,oy,odep
         else
            ln2k = 6371.*cos(elat(i)*dtr)*dtr
            olat = elat(i) - tmp1*cos(tmp2-strk(i)*dtr)/lt2k
            olon = elon(i) + tmp1*sin(tmp2-strk(i)*dtr)/ln2k
c            write(*,*) 'Origin (lat,lon) of subfault ',i,': ',olat,olon,odep
         endif
c Loop over displacement points
         do 100 j=1,m
            if (utm) then
               ...
               d = sqrt(x**2+y**2)
               az = 90.-atan2(y,x)/dtr
c               write(*,*) x,y,rlon(j),rlat(j),d,az,atan2(y,x)/dtr
c Use distance along sphere, with Cartesian approximation to fault coordinates
            else
c               write(*,*) rlon(j)-olon,rlat(j)-olat
               call distaz(6371.d0,olon,olat,rlon(j),rlat(j),d,az)
            endif
...
c     *                   U1,U2,U3,U11,U12,U21,U22,U31,U32)              01860000
 ...
 200  continue
      ...
      subroutine distaz(R,lon1,lat1,lon2,lat2,d,az)
c
c A quick and direty spherical distance and bearing calulator. Distance 
c returned in degrees, and azimuth is the bearing form lon1,lat1 to
c lon2,lat2
c
      IMPLICIT REAL*8 (A-H,L,O-Z)         
      real*8, intent(in):: R,lon1,lat1,lon2,lat2
      real*8, intent(out):: d,az
      pi = atan2(1.,1.)*4.
      dtr = pi/180.
      dlon = (lon2 - lon1)*dtr
      dlat = (lat2 - lat1)*dtr
      a = sin(dlat/2)**2 + cos(dtr*lat1)*cos(dtr*lat2)*sin(dlon/2)**2
      c = 2 * asin(min(1.d0,sqrt(a)))
      d = R * c
      az = atan2(sin((lon2-lon1)*dtr)*cos(lat2*dtr),
     &     cos(lat1*dtr)*sin(lat2*dtr)-sin(lat1*dtr)*cos(lat2*dtr)*
     &     cos((lon2-lon1)*dtr))
      az = 180.*az/pi
      return
      end
