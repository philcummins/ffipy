...
def read_velmod(velmod_filename):
    ...
    vmod[0:4,       0:nl      ] = np.array([float(x) for x in \
                     buf[3:nl*4+3]]).reshape((nl,4)).transpose()
    vmod[0:4,    nl:nl+nl1    ] = np.array([float(x) for x in \
                     buf[4*(nl+1):4*(nl+nl1+1)]]).reshape((nl1,4)).transpose()
    vmod[0:4,nl+nl1:nl+nl1+nl2] = np.array([float(x) for x in \
                     buf[4*(nl+nl1+1)+1:4*(nl+nl1+nl2+1)+1]]).reshape((nl2,4)).transpose()
    return {'tqp':tqp, 'tqs':tqs, 'ns':[nl,nl1,nl2], 'vmod':vmod}
...
def thetasr(lats,lons,latr,lonr):
    """
    ...
    """
    bazm,azm,dist = surf.thetasr(lats,lons,latr,lonr)    
    return bazm,azm,dist

def delay(az,p,da,dr,dz,v):
    """
    Wrapper for Kikuchi delay function
     -- computes delay times with respect to an origin -----------------
     -- assuming plane wave --------------------------------------------
    az   source to receiver azimuth
    p    horizontal slowness
    da   azimuth of subfault centroid from that of hypocenter subfault
    dr   distance of   ''  
    dz   depth of      ''
    v    p or s velocity at source depth
    """
    return kiklib.delay(az,p*v,da,dr,dz,v)
