...
c  f = ds(i)*g(i) , with ds(i) = 1
c si should be a complex array of length nt and elements (1.,0.)
c*********************************************************************
      subroutine kiksyn(sy,nt,dt,ib,ic,az,p,g,str,dip,rak,z,t12,
     +     tqp0,tqs0,vmod,nls,set_vmod)
      PARAMETER (ML0=10,PI=3.141593)
      logical,intent(in):: set_vmod
      real,intent(out):: sy(nt)
      real,intent(in):: vmod(4,*)
      real,intent(in):: dt,str,dip,rak,z,az,p,g,tqp0,tqs0
      integer,intent(in):: nls(3),ib,ic
      complex*8 sqp(nt),sqs(nt),si(nt)
      real so(2*nt),gr(2*nt),tqp,tqs
      COMMON /STR0/NL ,VP (ML0),VS (ML0),DEN (ML0),DEP (ML0)
      COMMON /STR1/NL1,VP1(ML0),VS1(ML0),DEN1(ML0),DEP1(ML0)
      COMMON /STR2/NL2,VP2(ML0),VS2(ML0),DEN2(ML0),DEP2(ML0)
      common /tq2/tqp,tqs
      do 25 i=1,nt
 25     sy(i) = 0.
      if (set_vmod) then
c     Unpack velocity model and set common block
         nl  = nls(1)
         nl1 = nls(2)
         nl2 = nls(3)
         tqp = tqp0
 ...
 100        dep(i)  = vmod(4,i)
         do 101 i=1,nl1
            ...
            vs1(i)  = vmod(2,nl+i)
            den1(i) = vmod(3,nl+i)
 101        dep1(i) = vmod(4,nl+i)
         do 102 i=1,nl2
            vp2(i)  = vmod(1,nl+nl1+i)
            vs2(i)  = vmod(2,nl+nl1+i)
            den2(i) = vmod(3,nl+nl1+i)
 102        dep2(i) = vmod(4,nl+nl1+i)
      endif
c Check for nt=power of 2
      x = log(real(nt))/log(2.)
      if (int(x).ne.x) then
         write(*,*) 'BODYW FAILS: nt must be a power of two!'
         stop
      endif
c Set attenuation factors
      df = 1./(dt*nt)
      call qfm(sqp,nt,tqp,df)
      call qfm(sqs,nt,tqs,df)
c         -- compute source time function -------------------------------
c Assume symmetric triangle ms=1, t1=t12,t2=t12
      call stime(so,nt,dt,1,t12,t12)
c     -- compute body waves -------------------------------------
c     dummy instrument response - result of instg call with nz=np=0
      do 200 i=2,nt/2
         f = (i-1)*df
         xtp = 1.
c This was a test for applying taper as in data decon
c         if (f.ge.0.25) then
c            xtp = 0.5*(1 + cos(4.*(f-0.25)*pi))
c         endif
c         write(*,*) f,xtp
         si(i) = cmplx(xtp,0.)
 200     si(nt-i+2) = cmplx(xtp,0.)
      si(1) = cmplx(0.,0.)
      si(nt/2+1) = cmplx(0.,0.)
c In Hong Kie's ffi program, units are adjust by 1.e4
      call bodyw(gr,nt,dt,ib,ic,str,dip,rak,z,az,p,g*1.e4,sqp,sqs,si)
c     -- convolve green's functions with the source time --------
      call conv(gr,so,sy,nt,nt,nt)
      do 205 i=1,nt
 205     sy(i) = sy(i)*dt
      return
      end
